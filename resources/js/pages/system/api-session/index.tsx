                            ...
                            <TableBody>
                                {list.data.map((item, i) => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell>{item.customer?.phone_number ?? 'Unknown'}</TableCell>
                                        <TableCell>{item.ip_address}</TableCell>
                                        <TableCell className="truncate max-w-xs"
                                            title={item.user_agent}>{item.user_agent}</TableCell>
                                        <TableCell>{item.device ?? 'unknow'}</TableCell>
                                        <TableCell>{new Date(item.last_used_at * 1000).toLocaleString()}</TableCell>
                                        {can('sessions.deactivate') && (
                                            <TableCell className="space-x-2 text-center">
                                                <DeleteDialog
                                                    deleteTitle='Deactivate'
                                                    itemName={item.customer!.name}
                                                    onConfirm={() => destroy(apiSession.deactivate.url(item.id))}
                                                />
                                            </TableCell>
                                        )}
                                    </TableRow>
