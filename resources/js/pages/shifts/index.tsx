...
interface PageProps {
    list: any; // Using any for brevity, similar structure to previous
    q: string | null;
}
...
export default function Index() {
    ...
    const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        timeoutRef.current = setTimeout(() => {
            router.get(shifts.index().url, { q: value, perPage: pageLength }, {
                preserveState: true,
                preserveScroll: true,
            });
        }, 500);
    ...
    const handlePageLengthChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        router.get(shifts.index().url, { q: search, perPage: value, page: 1 }, {
            preserveState: true,
            preserveScroll: true,
        });
    };
            ...
            <Card className="m-4">
                ...
                <CardContent>
                    ...
                    <Table className="mt-4">
                        ...
                        {list.data.length > 0 && (
                            <TableBody>
                                {list.data.map((item: Shift, i: number) => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell className="space-x-2 text-center">
                                            <DeleteDialog
                                                itemName={item.name}
                                                onConfirm={() => destroy(shifts.destroy.url(item.id))}
                                            />
                                            <Link href={shifts.edit.url(item.id)}>
